...
// получить все события по Экипажу Field "aircraftActivities" of type "AircraftActivitiesSection!" must have a sub selection.
	...
	КонецЕсли;
		...
		Если Ответ.Свойство("errors") Тогда
		    ...
		    СобытияОшибки = Новый Соответствие;
		    СобытияОшибки.Вставить("errors", Ответ.errors[0].message); 
				Если Не ТестЗапроса Тогда 
					ВызватьИсключение(Ответ.errors[0].message);
				КонецЕсли;

			Возврат СобытияОшибки;
		КонецЕсли;

	Если ТестЗапроса Тогда
		СобытияИнф = Новый Соответствие;
		СобытияИнф.Вставить("nformations", "Тест запроса пройден успешно, ошибок не обнаружено!"); 
		Возврат СобытияИнф; 
	КонецЕсли;
