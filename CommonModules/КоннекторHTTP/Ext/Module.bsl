...
// Отправляет POST запрос без получения ответа 
//
// Параметры:
//   URL - Строка - URL ресурса, к которому будет отправлен запрос.
//   Json - Структура, Соответствие - данные, которые необходимо сериализовать в JSON.
//   ДополнительныеПараметры - Структура - см. описание параметра в ВызватьМетодВСеансе.
Процедура PostJsonNoAnswer(URL, Json, ДополнительныеПараметры = Неопределено) Экспорт
	ВызватьHTTPМетод(Неопределено, "POST", URL, Неопределено, Json, Неопределено, ДополнительныеПараметры);
КонецПроцедуры
...
Функция ОтправитьHTTPЗапрос(Сессия, ПодготовленныйЗапрос, Настройки)
	...
	Попытка
		...
		//ОтветСтрокой = КакТекст(Ответ);
	Исключение
		Ответ = Неопределено;
		//ОтветСтрокой = ИнформацияОбОшибке().Описание;
	КонецПопытки;
	...
	//ЛогируемыеМетоды = Константы.ЛогироватьВебЗапросы.Получить();
	//КонецЗапроса = ТекущаяУниверсальнаяДатаВмиллисекундах();
	//СтруктураЗапроса = Новый Структура;
	//СтруктураЗапроса.Вставить("method", ПодготовленныйЗапрос.Метод);
	//
	//ДлительностьЗапроса = КонецЗапроса - НачалоЗапроса;
	//Если ЛогируемыеМетоды <> "" Тогда
	//	РегистрыСведений.ЛогиЗапросов.СоздатьЗаписьЛога(ПодготовленныйЗапрос.HTTPЗапрос, ОтветСтрокой,  ПодготовленныйЗапрос.Метод, ЛогируемыеМетоды, ДлительностьЗапроса, Ответ);
	//КонецЕсли;
...
Функция ОтправитьЗапрос(Сессия, ПодготовленныйЗапрос, Настройки)
	...
	ДлительностьЗапроса = КонецЗапроса - Начало;
	Если ЛогируемыеМетоды <> "" Тогда
		РегистрыСведений.ЛогиЗапросов.СоздатьЗаписьЛога(ПодготовленныйЗапрос.HTTPЗапрос, КакТекст(ПодготовленныйОтвет), ПодготовленныйЗапрос.Метод, ЛогируемыеМетоды, ДлительностьЗапроса, Ответ);
	КонецЕсли;
...
Функция ОбъектВJson(Объект, Знач ПараметрыПреобразования, Знач ПараметрыЗаписи) Экспорт
	...
	ЗаписатьJSON(ЗаписьJSON, Объект, НастройкиСериализации, "ФункцияПреобразования", КоннекторHTTP);
...
Функция JsonВОбъект(Json, Кодировка = "utf-8", ПараметрыПреобразования) Экспорт
