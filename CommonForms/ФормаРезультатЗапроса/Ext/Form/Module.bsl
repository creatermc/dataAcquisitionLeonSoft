	...
	КонецЕсли;
	    		...
	    		Для Каждого ЭлементА Из Параметры.РезультатВыполнения.data Цикл		
	    			...
	    			Для Каждого ЭлементБ из ЭлементА.Значение Цикл
						...
						Если ЭлементБ.Значение = Неопределено Тогда 
							СтрокаБ.ДанныеЗначения = "-";									
							Продолжить;
						КонецЕсли;
	    				
							Если ТипЗнч(ЭлементБ.Значение) = Тип("Массив") Или ТипЗнч(ЭлементБ.Значение) = Тип("Структура") Тогда
								СтрокаБ.ДанныеЗначения = ЭлементБ.Значение;									
								Продолжить;
							КонецЕсли;

	    				Для Каждого ЭлементВ из ЭлементБ.Значение Цикл
	    					 
							Если ТипЗнч(ЭлементВ) = Тип("КлючИЗначение") Тогда
								СтрокаВ = СтрокаБ.Строки.Добавить();

								Если ТипЗнч(ЭлементВ.Значение) = Тип("Массив") Или ТипЗнч(ЭлементВ.Значение) = Тип("Структура") Тогда
									СтрокаВ.Наименование = ЭлементВ.Ключ;
									СтрокаВ.ДанныеЗначения = ЭлементВ.Значение;									
									Продолжить;
								Иначе 
									СтрокаВ.Наименование = ЭлементВ.Ключ + ":  " + ЭлементВ.Значение;
									Продолжить;									
								КонецЕсли;
								...
								КонецЕсли;
	    							...
	    							Для Каждого ЭлементГ из ЭлементВ.Значение Цикл	    							
										...
										Если ТипЗнч(ЭлементГ.Значение) = Тип("Массив") Или ТипЗнч(ЭлементГ.Значение) = Тип("Структура") Тогда
