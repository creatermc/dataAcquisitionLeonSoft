...
Процедура ДобавитьВТекстЗапроса(Команда)
	...
	//	получить текущую строку в форме текствого поля "ИсходныйТекстДляФормированияЗапроса"
...
Функция ПроверитьНаПредыдущийЭлемент()
	...
	Для Индекс = 0 По СтрЧислоСтрок(Объект.ИсходныйТекстДляФормированияЗапроса) Цикл
	    ТекущCтрока = СтрПолучитьСтроку(Объект.ИсходныйТекстДляФормированияЗапроса, СтрЧислоСтрок(Объект.ИсходныйТекстДляФормированияЗапроса) - Индекс);
			//Если ТекущCтрока = "{" Тогда // "{"
			//	Объект.ТекстЗапроса = Объект.ТекстЗапроса + " {" + ПреносСтрокиКонсоль;
			//ИначеЕсли ТекущCтрока = "}" Тогда // "}"
			//	Объект.ТекстЗапроса = Объект.ТекстЗапроса + " }" + ПреносСтрокиКонсоль;
...
Процедура ЭлементыЗапросаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	РезултТекСтрока = ЭтаФорма.Элементы.ЭлементыЗапроса.ТекущиеДанные;
