...
//    Элементы.Редактор.ВыделенныйТекст = "Мое новое значение";
//КонецПроцедуры
	...
	Для Каждого ВыбранСтрока Из ВыбраннаяСтрока Цикл
		...
		ВставленныйТекст = ДокументЗапроса.Вставить(ЗакладкаНачало, ДобавочнТекст, ТипЭлементаФорматированногоДокумента.Текст);
		
		ВставленныйТекст = ДокументЗапроса.Вставить(ВставленныйТекст.ЗакладкаКонца, "", ТипЭлементаФорматированногоДокумента.ПереводСтроки);

		Элементы.ДокументЗапросаФорм.УстановитьГраницыВыделения(ВставленныйТекст.ЗакладкаКонца, ВставленныйТекст.ЗакладкаКонца); 
	КонецЦикла;

	Если РегРодитель Тогда
		ВставленныйТекст = ДокументЗапроса.Вставить(ЗакладкаНачало, Символы.ПС +  Символы.Таб + "}" + Символы.ПС + Символы.ПС, ТипЭлементаФорматированногоДокумента.ПереводСтроки);
	КонецЕсли;

	//ПозицияНачала = ДокументЗапроса.ПолучитьЗакладкуПоПозиции(Начало);
	//ПозицияОкончания = ДокументЗапроса.ПолучитьЗакладкуПоПозиции(Окончание);

    Элементы.ДокументЗапросаФорм.ПолучитьГраницыВыделения(ДокументЗапроса.ПолучитьЗакладкуКонца(),ДокументЗапроса.ПолучитьЗакладкуКонца());
	//Начало = ДокументЗапроса.ПолучитьПозициюПоЗакладке(ПозицияНачала);
	//Окончание = ДокументЗапроса.ПолучитьПозициюПоЗакладке(ПозицияОкончания);

	Элементы.ДокументЗапросаФорм.УстановитьГраницыВыделения(ДокументЗапроса.ПолучитьЗакладкуКонца(), ДокументЗапроса.ПолучитьЗакладкуКонца());


//ДокументЗапроса.ПолучитьЗакладкуНачала();
//ДокументЗапроса.ПолучитьЗакладкуПоПозиции(
//ДокументЗапроса.ПолучитьПозициюПоЗакладке(
//ДокументЗапроса.ПолучитьТекст(
//ДокументЗапроса.ПолучитьЗакладкуКонца()
...
Процедура ПолучитьВыделение(Команда)
    ПозицияНачала = 0;
    ПозицияОкончания = 0;
    Элементы.ДокументЗапросаФорм.ПолучитьГраницыВыделения(ПозицияНачала,ПозицияОкончания);
    Начало = ДокументЗапроса.ПолучитьПозициюПоЗакладке(ПозицияНачала);
    Окончание = ДокументЗапроса.ПолучитьПозициюПоЗакладке(ПозицияОкончания);
КонецПроцедуры
...
Процедура УстановитьВыделение(Команда)
    ПозицияНачала = ДокументЗапроса.ПолучитьЗакладкуПоПозиции(Начало);
    ПозицияОкончания = ДокументЗапроса.ПолучитьЗакладкуПоПозиции(Окончание);
    Элементы.ДокументЗапросаФорм.УстановитьГраницыВыделения(ПозицияНачала,ПозицияОкончания);
КонецПроцедуры   
