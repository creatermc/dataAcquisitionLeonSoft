...
&НаКлиенте
Процедура ЗащищенныеДанныеПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	Если Объект.Ссылка.Пустая() Тогда 
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияПредупреждение", ЭтотОбъект);
		ПоказатьПредупреждение(Оповещение, "Перед добавлением необходимо сохранить текущую карточку!", 0, "Внимание!");
		Отказ = Истина;
	КонецЕсли;
КонецПроцедуры
